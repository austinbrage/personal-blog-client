...
export function ModalDelete({ mode, modalRef }: Props) {
    ...
    const handleDelete = () => {
        ...
        mode === 'article' && deleteArticle()     
        mode === 'section' && deleteSection()     
        closeModalDelete()   
    }

    useEscape({
        menuRef: modalRef,
        closeMenu: closeModalDelete
    })
        ...
        <div id="popup-modal-1" tabIndex={-1} ref={modalRef} className="hidden transition-all overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div className="relative p-4 w-full max-w-md max-h-full">
                <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <button onClick={closeModalDelete} type="button" className="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal">
