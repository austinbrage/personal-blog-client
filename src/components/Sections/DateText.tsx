...
export function DateText({ type, date }: Props) {

    const dateConverted = new Date(date)
    const dateNormalized = dateConverted.toLocaleDateString('en-US', dateConfig).split(', ')

    return (
        <div className="inline-flex px-5 py-3 my-3 italic max-w-max text-gray-700" aria-label="Breadcrumb">
            <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                <li className="inline-flex items-center">
                    <span className="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
                        <p className="me-3 font-bold text-lg">{type}</p> 
                ...
                <li aria-current="page">
                    <div className="flex items-center">
                        ...
                        <span className="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">
                            {dateNormalized[2]}
                            {type === 'Created at' && <span className="ps-10">-</span>}
                        </span>
                    </div>
                </li>
            </ol>
