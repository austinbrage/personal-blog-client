            ...
            
            {
                Object.entries(familyOptions).map(([marginName, sizeValue], index) => (
                    <div 
                        ...
                        className={`
                            ${(currentFamily === sizeValue) ? 'opacity-100 scale-105' : 'opacity-70 scale-100'}
                            p-2 w-24 text-center text-lg font-semibold cursor-pointer rounded-md backdrop-blur-md text-white bg-[rgba(0,0,0,0.3)] transition-all hover:opacity-100 hover:scale-105
                        `.trim()}
                    >
                        {marginName}
                    </div>
