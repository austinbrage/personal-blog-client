import { useUserAuthor } from '../../hooks/useUser'
...
export function ModalAuthor({ modalRef, toggleModal, isToggle, currentAuthor }: Props) {

    const [author, setAuthor] = useState<string>(currentAuthor)
    const { isSuccess, isPending, editUserAuthor } = useUserAuthor()

    useEffect(() => {
        setAuthor(currentAuthor)
    }, [currentAuthor, isToggle])
    ...
    return (
        <div ref={modalRef} id="crud-modal-5" tabIndex={-1} aria-hidden="true" className="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    ...
                    <form onSubmit={handleSubmit} className="p-4 md:p-5">
                        ...
                        <div className="grid gap-4 mb-4 grid-cols-2">
                            <div className="col-span-2">
                                <label htmlFor="author3" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                    User author name
                                </label>
                                <input 
                                    required
